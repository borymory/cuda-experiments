...
namespace FlashLab::Reduction {
    ...
    template<const int BN>
    __global__ void rowMaxXOR_v2 (float *B, const int N, const int d) {
        // We launch CEIL_DIV(N, BN) many blocks
        // We have BN * 32 many threads per block
        ...
        if (rowIdx + ty < N) {
            ...
            for (unsigned int offset = 0; offset < d; offset += 32) {
                // If d is not a multiple of 32
                if (tx + offset < d)
                    threadVal = fmaxf(threadVal, B[ty * d + (tx + offset)]);
            }

            // Start XOR shuffle:
            for (unsigned int mirrorIdx = 1; mirrorIdx <= 16; mirrorIdx <<= 1) {
                threadVal = fmaxf(threadVal, __shfl_xor_sync(FULL_MASK, threadVal, mirrorIdx));
            }
